...
> **현재 버전:** [1.4.1](https://github.com/gesori-pro/PSO2_Shape_Studio/releases/tag/v1.4.1).
...
- NGS 캐릭터 크리에이터와 같은 방식(태양·보조광·카메라 조명, 환경광, 글레어,
  톤 커브)으로 캐릭터를 비추는 데모 조명 미리보기를 선택적으로 사용해 피부,
  광택, 표면 디테일을 살펴볼 수 있습니다.
- 캐릭터 파일의 근육 수치를 피부 텍스처에 적용합니다. 피부의 젖음 및 건조 표현은
  실험 단계이며 계속 구현하고 검증하고 있습니다.
- NGS 방식으로 뷰포트를 조작할 수 있습니다. 좌클릭 드래그는 카메라, 우클릭
  드래그는 고정 조명을 기준으로 캐릭터를 회전하며, Ctrl+드래그는 상하 이동,
  휠클릭은 시점과 캐릭터 초기화, 휠은 확대 및 축소입니다.
