...
[English](README.md) | [日本語](README.ja.md) | **한국어** | [简体中文](README.zh-Hans.md) | [繁體中文](README.zh-Hant.md)
...
> **현재 버전:** [1.3.1](https://github.com/gesori-pro/PSO2_Shape_Studio/releases/tag/v1.3.1).
> 최신 자체 포함 Windows x64 빌드는 릴리스 페이지에서 다운로드할 수 있습니다.

## 기능

- `.aqp`, `.aqn`, `.ice`에서 PSO2 모델을 열 수 있습니다.
- 성별·종족별 캐릭터 커스터마이즈 파일(`.fdp`, `.fnp`, `.fhp`, `.fcp`, `.mdp`,
  `.mnp`, `.mhp`, `.mcp`)과 암호화되지 않은 변형 파일을 불러와 체형과 색상을
  적용할 수 있습니다. 남성과 여성은 각자의 체형 비율 대응표를 사용합니다.
...
- 애플리케이션 UI를 영어(글로벌), 일본어, 한국어, 중국어 간체, 중국어 번체로 전환할
  수 있습니다.
...
5. S/P/R 슬라이더를 편집하거나 숫자를 직접 입력합니다. 옵션 창에서는 기본 그룹을
   ...
   초기화합니다.
6. 편집 결과를 `_sa.aqm` 파일로 저장합니다. 게임에 반영하려면 다음 절의
   리패킹 순서를 따르세요.

## 저장한 _sa.aqm을 게임에 적용하기

Shape Studio가 저장하는 `_sa.aqm`은 의상 하나에만 적용되는 체형 보정
파일입니다. 게임은 이 파일을 단독으로 읽지 않으며, 해당 의상의 ICE 아카이브
안에 있는 원본 `..._sa.aqm` 항목을 교체해야만 반영됩니다. 아래 순서는 커뮤니티
ICE 도구인 [Zamboni](https://github.com/Shadowth117/Zamboni)로 리패킹하는
방법입니다.

1. **원본 항목 이름 그대로 저장합니다.** 의상을 먼저 불러온 뒤 *_sa.aqm
   저장…*을 누르면, 저장 대화상자가 불러온 모델에서 파생한 올바른 이름을
   제안합니다(예: `pl_rbd_201630_bw.aqp` → `pl_rbd_201630_bw_sa.aqm`).
   이 이름을 그대로 유지하세요. 다른 이름으로 저장하면 아무것도 교체되지
   않습니다.
2. **의상의 게임 파일 위치를 확인합니다.** 검색 결과에 마우스를 올리면 해당
   항목이 가리키는 ICE 아카이브의 전체 경로(`pso2_bin/data/...` 아래의 32자리
   파일 이름)가 표시됩니다. 불러온 모델 목록에서도 같은 경로를 툴팁으로 볼 수
   있습니다. 이 파일을 `C:\work` 같은 짧은 작업 폴더로 복사하세요 — 게임 폴더
   안의 파일을 직접 수정하면 안 됩니다.
3. **복사본을 Zamboni로 풉니다.** 복사한 파일을 Zamboni에 끌어다 놓으면
   압축이 풀립니다. 모델 데이터는 `group2` 폴더에 들어 있으며, 원본
   `..._sa.aqm`도 여기에 있습니다.
4. **항목을 교체합니다.** `group2` 안의 `..._sa.aqm`을 저장해 둔 파일로
   덮어씁니다. 압축 해제된 폴더 안에 백업 사본을 두지 마세요 — 폴더 안의
   모든 파일이 그대로 다시 패킹됩니다.
5. **리패킹합니다.** 압축 해제된 폴더를 Zamboni로 다시 패킹하고, 설치 전에
   결과 파일 이름이 원래의 32자리 이름인지 확인하세요(필요하면 이름을
   바꿉니다). 작업 경로가 너무 길면 리패킹이 조용히 실패할 수 있어서 2단계의
   짧은 폴더가 중요합니다.
6. **설치합니다.** 원본을 자동으로 백업하고 복원할 수 있는
   [PSO2NGS Mod Manager](https://github.com/KizKizz/pso2_mod_manager) 같은
   모드 매니저 사용을 권장합니다. 수동으로 설치한다면 원본 게임 파일을 직접
   백업한 뒤 리패킹한 아카이브로 덮어쓰세요.

참고:

- 보정은 리패킹한 그 의상에만 적용됩니다.
- 게임 업데이트가 원본 파일을 복원할 수 있습니다. 업데이트 후에는 모드를
  다시 설치하세요.
- 설치 전에 리패킹한 ICE를 Shape Studio로 열어 결과를 미리 확인할 수
  있습니다.
- 게임 파일 수정에 따른 책임은 본인에게 있습니다.
