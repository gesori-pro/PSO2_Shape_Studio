...
> **当前版本：** [1.4.0](https://github.com/gesori-pro/PSO2_Shape_Studio/releases/tag/v1.4.0)。
...
- 可选用演示光照预览，以与 NGS 角色创建相同的方式（太阳光、补光与镜头灯、
  环境光、眩光与色调曲线）照亮角色，检查皮肤、光泽与表面细节。
- 将角色文件中的肌肉数值应用到皮肤贴图。皮肤的湿润与干燥表现仍处于实验
  阶段，正在继续实现与验证。
- 支持 NGS 风格的视口操作：左键拖动旋转镜头，右键拖动让角色相对固定灯光
  旋转，Ctrl+拖动垂直移动，中键重置视角与角色，滚轮缩放。
