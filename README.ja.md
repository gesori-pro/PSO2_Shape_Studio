...
> **現在のバージョン：** [1.0.2](https://github.com/gesori-pro/PSO2_Shape_Studio/releases/tag/v1.0.2)。
> 最新の自己完結型Windows x64ビルドは、リリースページからダウンロードできます。

## 機能

- `.aqp`、`.aqn`、`.ice`からPSO2モデルを開けます。
- 旧PSO2の種族別キャラクタークリエイトデータ（`.fdp`、`.fnp`、`.fhp`、`.fcp`）と
  暗号化されていない派生ファイルを読み込み、体型とカラーを適用できます。
- コスチュームの体型補正モーション（`_sa.aqm`）を読み込み・保存できます。
- `body_root`のYスケールで足裏の高さを0.01刻みで調整し、表示を切り替えられる
  ワールド床ガイドと比較できます。
