...
> **目前版本：** [1.4.1](https://github.com/gesori-pro/PSO2_Shape_Studio/releases/tag/v1.4.1)。
> 請從發佈頁面下載最新的免安裝 Windows x64 版本。

## 功能

- 開啟 `.aqp`、`.aqn`、`.ice` 來源的 PSO2 模型檔案。
- 載入依性別與種族區分的角色外觀檔（`.fdp`、`.fnp`、`.fhp`、`.fcp`、`.mdp`、
  `.mnp`、`.mhp`、`.mcp`）及其未加密變體，並套用其中的體型比例與顏色。
  男性與女性各自使用專用的體型比例表。
...
- 可選用示範光照預覽，以與 NGS 角色創建相同的方式（太陽光、補光與鏡頭燈、
  環境光、眩光與色調曲線）照亮角色，檢查皮膚、光澤與表面細節。
- 將角色檔案中的肌肉數值套用至皮膚貼圖。皮膚的濕潤與乾燥表現仍處於實驗
  階段，正在持續實作與驗證。
- 支援 NGS 風格的檢視區操作：左鍵拖曳旋轉鏡頭，右鍵拖曳讓角色相對固定
  照明旋轉，Ctrl+拖曳上下移動，中鍵重設視角與角色，滾輪縮放。
